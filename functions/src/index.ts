...
const genAI = new GoogleGenAI(functions.config().gemini.api_key);
...
export const updateWorkerLocation = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const autoAssignOrder = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "User must be authenticated");
  }
  ...
  try {
    ...
    const workers = workersQuery.docs.map(doc => ({
      id: doc.id,
      ...doc.data(),
    }));
...
export const processPendingOrders = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    for (const orderDoc of pendingOrdersQuery.docs) {
      try {
        await autoAssignOrder({ orderId: orderDoc.id }, { auth: { uid: "system" } } as any);
      } catch (error) {
...
async function sendPushNotification(notification: any): Promise<void> {
  ...
  await realtimeDb.ref(`notifications/${notification.userId}`).push({
    ...notification,
    timestamp: Date.now(),
    read: false,
  });
...
async function saveNotificationToDb(notification: any): Promise<void> {
...
async function findBestWorker(workers: any[], orderData: any): Promise<any> {
  // Implement intelligent scoring algorithm
  const customerLocation = orderData.customerInfo?.location;
  ...
  for (const worker of workers) {
    ...
    if (worker.location) {
      const distance = calculateDistance(
        ...
        worker.location._latitude,
        worker.location._longitude
      );
